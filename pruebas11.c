...
char	*ft_initializing(int fd, char *rembuffer, int *nl_pos, int *byt_read)
{
	...
	*byt_read = read(fd, str, BUFFER_SIZE);
	...
	if (*byt_read)
		str[*byt_read] = '\0';
	printf("retorno initializing --> str: %s\n", str);
	return (str);
}
...
char	*ft_get_line(char *r_buffer, int *byt_read)
{
	...
	while (++i < end_line)
		line[i] = r_buffer[i];
	line[end_line] = '\0';
	printf("retorno get_line --> line: %s\n", line);
	return (line);
}

char	*ft_clean_rem(char *rbuffer, int *nl_pos)
{
	const int		len_rbuffer = ft_strlen(rbuffer);
	const int		beg_buff = ft_check_nl(rbuffer);
	char			*tmp;
	int				i;
...
char	*get_next_line(int fd)
{
	...
	{
		temp = ft_strjoin(rem_buffer, tmp_buffer);
		free(rem_buffer);
		rem_buffer = temp;
	}
	printf("11]]]] rem_buffer= %s\n", rem_buffer);
// OJO PUEDE HABER PROBLEMAS EN EL STRJOIN DE 2 FUNCIONES NULL OJO
	...
	printf("22]]]] rem_buffer= %s\n", rem_buffer);
	line = ft_get_line(rem_buffer, &byt_read);
	printf("33]]]] line= %s", line);
